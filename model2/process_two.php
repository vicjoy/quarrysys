...
if ((isset($_SESSION['hasVoted']))) {
	...
	if (isset($_GET['Submit1']) && isset($_GET['q'])) {
		...
		if ($db_found) {
			...
			if($selected_radio == "A") {
				$votedSQL = "UPDATE qes_table SET Strongly_Agree = Strongly_Agree + 1 WHERE ID = ?";
				$voteMessage = insert_vote($db_found, $votedSQL, $idNumber);
			}
			else if($selected_radio == "B"){
				$votedSQL = "UPDATE qes_table SET Agree = Agree + 1 WHERE ID = ?";
				$voteMessage = insert_vote($db_found, $votedSQL, $idNumber);
			}
			else if($selected_radio == "C"){
				$votedSQL = "UPDATE qes_table SET Disagree = Disagree + 1 WHERE ID = ?";
				$voteMessage = insert_vote($db_found, $votedSQL, $idNumber);
			}
			else if($selected_radio == "D"){
				$votedSQL = "UPDATE qes_table SET Strongly_Disagree = Strongly_Disagree + 1 WHERE ID = ?";
				$voteMessage = insert_vote($db_found, $votedSQL, $idNumber);
			}
			else {
...
function insert_vote($db, $sql, $id) {
	...
	//header('survey2.php'); 
	echo "<script>window.location='../qms2/qms_three.php'</script>";
}
